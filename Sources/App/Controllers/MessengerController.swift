                ...
                if !postback.isEmpty {
                    ...
                    if text.characters.count <= 3 {
                        ...
                        if !auditoriums.isEmpty {
                            let message = try Messenger.buttons(auditoriums, title: "Аудиторії")
                            try self.sendResponse(response: message, senderID: senderID)
                        }
                        // Groups
                        var groups: [Button] = []
                        ...
                        if !groups.isEmpty {
                            let message = try Messenger.buttons(groups, title: "Групи")
                            try self.sendResponse(response: message, senderID: senderID)
                        }
                        // Teachers
                        var teachers: [Button] = []
                        ...
                        if !teachers.isEmpty {
                            let message = try Messenger.buttons(teachers, title: "Викладачі")
                            try self.sendResponse(response: message, senderID: senderID)
                        }
